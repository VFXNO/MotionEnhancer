...
// MSAD64 block matcher. One threadgroup per 16x16 flow block: the C++ side
...
// region, preserving full 16x16 spatial context without extra sample cost.
...
static const float kAdaptiveRefineScore = 0.85f;
static const float kStaticTieScore = 0.90f;
static const float kStaticTieMargin = 0.01f;
                  ...
                  out float bestScore, out float2 bestVector)
{
    ...
    for (uint c = li; c < candidateCount; c += kThreads)
    {
        ...
        for (int y = 0; y < support; ++y)
        {
            ...
            for (int x = 0; x < support; ++x)
            {
                float cv = CandWindow[w.y + y * sampleStep][w.x + x * sampleStep];
                candSum += cv;
            }
        }
        ...
        for (int y = 0; y < support; ++y)
        {
            ...
            for (int x = 0; x < support; ++x)
            {
                float cv = CandWindow[w.y + y * sampleStep][w.x + x * sampleStep] - candMean;
...
void CSMain(uint3 groupId : SV_GroupID, uint3 tid : SV_GroupThreadID, uint li : SV_GroupIndex)
{
    ...
    if (li == 0)
    {
        ...
        // MSAD has many near-ties on static UI. Prefer zero only when the
        // match is low-confidence and zero is effectively equivalent; strong
        // moving-object matches remain untouched.
        bool useZero = zeroDistinct &&
                       ((bestP < kStaticTieScore && bestZ >= bestP - kStaticTieMargin) ||
                        bestZ > bestP);
    ...
    // A strong 16x16 match is already stable. Reserve the smaller support
    // refinement for weak or ambiguous blocks where detail is needed.
    if (RefinementBaseScore >= kAdaptiveRefineScore)
    {
        if (li == 0) OutputFlow[groupId.xy] = RefinementBase;
        return;
    }
